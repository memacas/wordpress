	...
	class Ivole_Reviews {
	  ...
	  public function __construct() {
			...
			if( 'yes' == get_option( 'ivole_reviews_verified', 'no' ) ) {
				add_action( 'wp_enqueue_scripts', array( $this, 'ivole_style_3' ) );
				if ( ( version_compare( WC()->version, "2.2.11", ">=" ) ) && ( version_compare( WC()->version, "2.5", "<" ) )  ) {
					add_filter( 'wc_get_template', array( $this, 'compatibility_reviews'), 10, 5 );
		...
		public function custom_fields_attachment( $comment_form ) {
			...
			$html_field_attachment .= '</label><input type="file" multiple="multiple" name="review_image_' . $post_id . '[]" id="review_image" />';
			...
			if( 0 === intval( $comment->comment_parent ) ) {
				...
				if( '' !== $order_id ) {
					...
					if( intval( $shop_page_id ) === intval( $product_id ) ) {
						$output .= ' - <a href="https://www.cusrev.com/reviews/' . get_option( 'ivole_reviews_verified_page', Ivole_Email::get_blogdomain() ) . '/s/r-' . $order_id;
					} else {
					...
					$output .= '<img src="' . untrailingslashit( plugin_dir_url( __FILE__ ) ) . '/img/external-link.png" alt="' . __( 'External link', IVOLE_TEXT_DOMAIN ) . '" class="ivole-verified-badge-ext-icon"></span></p>';
					echo $output;
				}
			}
		}
